...
macro_rules! impl_tiled_matrix {
    ($s:ty) => {
        impl TiledMatrix<$s>
        {
            ...
            ///
            /// let matrix = TiledMatrix::<f64>::new(100, 200, 1.0);
            /// ```
            pub fn new(nrows: usize, ncols: usize, init: $s) -> Self {
                ...
                |(nrows, ncols, init)| { Tile::<$s>::new(*nrows, *ncols, *init) }
                ).collect();
            ...
            /// let matrix = TiledMatrix::<f64>::from(&flat_matrix, 100, 200, 100);
            /// ```
            pub fn from(other: &[$s], nrows: usize, ncols: usize, lda: usize) -> Self {
                ...
                // Parallel generation of tiles
                let tiles = tiles.par_iter().map(
                |(shift, nrows, ncols, lda)| { Tile::<$s>::from(&other[*shift..], *nrows, *ncols, *lda) }
                ).collect();
            ...
            /// let tiled_matrix = TiledMatrix::<f64>::new(100, 200, 1.0);
            ...
            /// 2D-array of tiles.
            #[inline]
            pub fn get_tile(&self, i: usize, j: usize) -> &Tile<$s> {
            ...
            /// 2D-array of tiles.
            #[inline]
            pub fn get_tile_mut(&mut self, i: usize, j: usize) -> &mut Tile<$s> {
                assert!(i < self.nrows() && j < self.ncols());
                match self.transposed {
            ...
            pub fn gemm_tiled_mut(transa: bool, transb: bool, m: usize, n: usize, k: usize, alpha: $s,
                                    a: &[$s], lda:usize, b: &[$s], ldb: usize, beta: $s, c: &mut [$s], ldc: usize)
            {
                ...
                for jj in 0..n_tiles {
                    ...
                    for ii in 0..m_tiles {
                        ...
                        let mut c_tile = {
                            if beta == zero {
                                Tile::<$s>::new(c_nrows, c_ncols, zero)
                            } else {
                                Tile::<$s>::from(&c[c_start..], c_nrows, c_ncols, ldc)
                            }
                        };
                        ...
                        for kk in 0..k_tiles {
                            ...
                            let a_tile = Tile::<$s>::from(&a[a_start..], a_nrows, a_ncols, lda);
                            let b_tile = Tile::<$s>::from(&b[b_start..], b_nrows, b_ncols, ldb);
                            Tile::<$s>::gemm_mut(alpha, &a_tile, &b_tile, one, &mut c_tile);
                        }
        ...
        impl std::ops::IndexMut<(usize,usize)> for TiledMatrix<$s>
        {
            ...
            /// use samo::tiled_matrix::TiledMatrix;
            ///
            /// let mut matrix = TiledMatrix::<f64>::new(64, 64, 1.0);
            /// matrix[(0, 0)] = 2.0;
...
mod tests {
    ...
    fn copy_in_vec() {
        ...
        let matrix = TiledMatrix::<f64>::from(&other, m, n, lda);
    ...
    #[test]
    fn number_of_tiles() {
        let matrix = TiledMatrix::<f64>::new(2*TILE_SIZE, 10, 0.);
        assert_eq!(matrix.nrows_tiles, 2);
        let matrix = TiledMatrix::<f64>::new(2*TILE_SIZE+1, 10, 0.);
        assert_eq!(matrix.nrows_tiles, 3);
        let matrix = TiledMatrix::<f64>::new(10, 2*TILE_SIZE, 0.);
    ...
    fn transposition() {
        ...
        let a = TiledMatrix::<f64>::from(&a, m, n, m);
        let a_t = TiledMatrix::<f64>::from(&a_t, n, m, n);
    ...
    fn test_dgemm() {
        ...
        // Tiled matrices
        let c_ref = TiledMatrix::<f64>::from(&c_ref, m, n, m);
        let c_ref_t = TiledMatrix::<f64>::from(&c_ref_t, n, m, n);
        ...
        let a = a.t();
        let b = b.t();
        let c_t = TiledMatrix::<f64>::gemm(2.0, &b, &a);
